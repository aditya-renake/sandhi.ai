...
export default function MovementAnalysis() {
  ...
  const sittingHeadYRef = useRef(0.48)
  const standingHeadYRef = useRef(0.22)
  const currentHeadYRef = useRef(null)
  const hasCalibratedBaselinesRef = useRef(false)
  const smoothHeadYRef = useRef(null)
  const smoothElevationRef = useRef(0.18)
  const lastPostureRef = useRef("SITTING")
  const hasStoodUpRef = useRef(false)
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.code === "Space" && e.target.tagName !== "INPUT" && e.target.tagName !== "BUTTON" && e.target.tagName !== "TEXTAREA") {
        ...
        if (newPosture === "STANDING") {
          hasStoodUpRef.current = true
          playPleasantChime()
        } else if (newPosture === "SITTING") {
          hasStoodUpRef.current = false
          const now = Date.now()
          if (!testCompleteRef.current && now - lastRepTimeRef.current > 600) {
  ...
  const triggerStartTest = (useWebcam = true) => {
    ...
    hasCalibratedBaselinesRef.current = false
    smoothHeadYRef.current = null
    if (currentHeadYRef.current !== null) {
      sittingHeadYRef.current = currentHeadYRef.current
      standingHeadYRef.current = Math.max(0.08, currentHeadYRef.current - 0.16)
      hasCalibratedBaselinesRef.current = true
    }
    playPleasantChime()
    console.log("[REP-DEBUG][TEST-START] ⏱️ triggerStartTest invoked: countdown starting (3s). useWebcam=", useWebcam)
    setCountdown(3)
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        console.log("[REP-DEBUG][TEST-GO] 🚀 'GO!' reached: isTestStarted=true, testComplete=false, repCount=0, lastPostureRef=", lastPostureRef.current)
        if (!useWebcam) {
          launchSimulation()
        } else {
          if (animFrameId.current) cancelAnimationFrame(animFrameId.current)
  ...
  const handleMediaPipeResults = (results) => {
    ...
    // 1. Occlusion / Visibility Evaluation (warning only, do NOT abort head tracking!)
    const visEval = evaluateLegVisibility(results.poseLandmarks, "auto")
    if (!visEval.isValid) {
      setOcclusionWarning("⚠️ Keep hip and knee in camera view")
    } else {
      setOcclusionWarning("")
    }
    ...
    } else if (visEval.hip && visEval.knee) {
  ...
  const processFrame = (simulatedProgress = null) => {
    ...
    // Send frame to MediaPipe Pose detector if loaded (guarded against concurrent WASM frame collisions)
    if (isLiveWebcam && poseInstanceRef.current && videoRef.current.readyState >= 2 && videoRef.current.videoWidth > 0) {
    ...
    if (isLiveWebcam) {
      // Prioritize MediaPipe landmark head position if recently updated, otherwise optical skin tracker
      const isMediaPipeHeadFresh = currentHeadYRef.current !== null && (Date.now() - lastMediaPipeAngleTimeRef.current < 1500)
      if (isMediaPipeHeadFresh) {
        observedHeadY = currentHeadYRef.current
      } else {
        const opticalY = analyzeWebcamBodyY(videoRef.current)
        if (opticalY !== null) {
          observedHeadY = opticalY
        } else if (currentHeadYRef.current !== null) {
          observedHeadY = currentHeadYRef.current
        }
      }

      if (observedHeadY !== null) {
        // High-responsiveness head tracking (alpha 0.75) for real-time instantaneous tracking
        smoothHeadYRef.current = smoothHeadYRef.current === null
          ? observedHeadY
          : 0.25 * smoothHeadYRef.current + 0.75 * observedHeadY
        const effectiveHeadY = smoothHeadYRef.current

        // Dynamic Baseline Auto-Calibration:
        // On first detection, calibrate sitting baseline directly to user's seated head level
        if (!hasCalibratedBaselinesRef.current || sittingHeadYRef.current === null) {
          sittingHeadYRef.current = effectiveHeadY
          standingHeadYRef.current = Math.max(0.06, effectiveHeadY - 0.18)
          hasCalibratedBaselinesRef.current = true
        }

        // Dynamically expand sitting baseline downwards when user sits lower (valley: larger Y)
        if (effectiveHeadY > sittingHeadYRef.current) {
          sittingHeadYRef.current = effectiveHeadY
        }

        // Dynamically expand standing baseline upwards when user stands higher (peak: smaller Y)
        if (effectiveHeadY < standingHeadYRef.current) {
          standingHeadYRef.current = effectiveHeadY
        }

        const span = Math.max(0.09, sittingHeadYRef.current - standingHeadYRef.current)
        // High head (standing) -> 1.0; Low head (sitting) -> 0.0
        const rawElev = Math.max(0, Math.min(1, (sittingHeadYRef.current - effectiveHeadY) / span))

        // Fast real-time elevation response (alpha 0.80)
        smoothElevationRef.current = 0.20 * smoothElevationRef.current + 0.80 * rawElev
    ...
    // ── STRICT REAL-TIME REP STATE MACHINE (NO MULTI-COUNTING, NO BURSTS) ──
    // Standing is recognized when user rises past 52% elevation OR knee angle >= 136°
    // Sitting is recognized when user descends below 36% elevation OR knee angle <= 122°
    const isStandingPosition = elevation >= 0.52 || (kneeAngleRef.current && kneeAngleRef.current >= 136)
    const isSittingPosition = elevation <= 0.36 || (kneeAngleRef.current && kneeAngleRef.current <= 122)

    let currentPosture = lastPostureRef.current

    // Phase 1: User stands up -> arm the rep
    if (isStandingPosition) {
      if (!hasStoodUpRef.current) {
        hasStoodUpRef.current = true
        lastPostureRef.current = "STANDING"
        currentPosture = "STANDING"
        setSitToStandState("STANDING")
        playPleasantChime()
        console.log(`[REALTIME-REP] 🟢 STANDING confirmed (elevation=${(elevation * 100).toFixed(0)}%, angle=${kneeAngleRef.current || 0}°). Next sit will complete the rep.`)
      }
    } 
    // Phase 2: User sits down -> if they previously stood up, complete EXACTLY 1 rep!
    else if (isSittingPosition) {
      ...
      if (hasStoodUpRef.current) {
        const now = Date.now()
        if (now - lastRepTimeRef.current >= 850) {
          hasStoodUpRef.current = false // CONSUMED! Cannot increment again until user stands back up!
          lastRepTimeRef.current = now
          repCooldownRef.current = now
          ...
          setRepCount((prevReps) => {
            ...
            console.log(`[REALTIME-REP] 🎯 REAL-TIME REP COUNTED: ${prevReps} -> ${nextReps}`)
            playPleasantChime()
    ...
    const effectiveHead = smoothHeadYRef.current || observedHeadY
    const displayHeadY = (effectiveHead || (sittingHeadYRef.current - elevation * (sittingHeadYRef.current - standingHeadYRef.current))) * height
    ctx.save()
    ...
    const isAtStand = elevation >= 0.52
    const isAtSit = elevation <= 0.36
    const lineColor = isAtStand ? "#10b981" : isAtSit ? "#f59e0b" : (hasStoodUpRef.current ? "#34d399" : "#38bdf8")
    ...
    ctx.font = "bold 11px Inter, sans-serif"
    const statusLabel = isAtStand 
      ? "▲ STAND LEVEL REACHED" 
      : isAtSit 
        ? "▼ SIT LEVEL" 
        : hasStoodUpRef.current 
          ? `👤 Returning to Sit (${Math.round(elevation * 100)}%)` 
          : `👤 Head (${Math.round(elevation * 100)}%)`
    ctx.fillText(statusLabel, width - 180, displayHeadY - 5)
    ctx.restore()
